...
def read_excel_data(demand_root,distance_root):
    ...
    for i in range(sheet_demand.nrows):                                         #按行读取需求数据
        Demand_matrix_table.append(sheet_distance.row_values(i))                #保存行需求数据于需求矩阵表中
...
#模拟退火算法
def SA(T_begin,T_end,alpha,L,Distance_matrix):                                  #T_begin初始温度;T_end终止温度;alpha降温系数;L迭代次数
    tabu_list = []                                                              #空禁忌列表
    ...
    while T > T_end:                                                            #如果当前温度大于终止温度
        for i in range(L):                                                      #循环L次
            ...
            delta_dist = new_dist - old_dist                                    #计算新旧路线长度的差值
            if delta_dist >= 0:                                                 #如果差值≥0，说明新路线比旧路线长，按一定概率接受新路线
                rand = random.uniform(0,1)
                if rand < math.exp(-delta_dist/T):
                    old_route = new_route
            else:                                                               #否则，必然接受新路线
                old_route = new_route
        T = T * alpha                                                           #降温
        ...
        print(cnt,"次降温，温度为：",T," 路程长度为：", Shortestlength)               #报文
...
def main():
    ...
    for i in range(independent_runs):                                           #独立运行 independent_runs 次
        optimal_distance,optimal_route = SA(T_begin,T_end,alpha,L,Distance_matrix)#运行SA()，输出当次运行的最优长度和最优路线
