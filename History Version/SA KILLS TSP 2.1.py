...
def SA(T_begin,T_end,alpha,L,Distance_matrix,Demand_matrix):                    #T_begin初始温度;T_end终止温度;alpha降温系数;L迭代次数
    ...
    plt.savefig('SA fitness (5 times)' + str(T_begin) + ' ' + str(T_end) + ' ' + str(alpha) + ' ' + str(L) + '.png')#保存到项目目录下命名为XXX
...
def main():
    ...
    for i in range(independent_runs):                                           #独立运行 independent_runs 次
        ...
        sheet.write(i+1,3,optimal_demand)
        sheet.write(i+1,4,optimal_fitness)
    work_book.save('SA ('+str(independent_runs)+' times).xls')                                   #保存工作表于项目目录下，命名为XXX
    plt.clf()                                                                   #清空画板
    plt.plot(Several_independent_runs_fitness_list)                            #绘制若干次独立运行的最优路线长度折线图
    plt.savefig('SA ('+str(independent_runs)+' times)')                                          #保存图片于项目目录下，命名为XXX
