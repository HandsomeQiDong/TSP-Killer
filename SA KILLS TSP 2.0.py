'''
在SA+Tabu的基础上，修改第102行代码，持续清空禁忌列表；修改第132行代码，保存图片重命名；修改第155行代码，重命名
...
def fitness(route_distance, sum_demand):
    ...
    return 0.3*route_distance-7*sum_demand
...
def SA(T_begin,T_end,alpha,L,Distance_matrix,Demand_matrix):                    #T_begin初始温度;T_end终止温度;alpha降温系数;L迭代次数
    ...
    plt.savefig("SA KILLS TSP A single run fitness(1 times).png")                        #保存到项目目录下命名为XXX
    return optimal_distance,optimal_route,optimal_demand,optimal_fitness                                       #输出最优路程长度和最优路径
...
def main():
    ...
    for i in range(independent_runs):                                           #独立运行 independent_runs 次
        optimal_distance,optimal_route,optimal_demand,optimal_fitness = SA(T_begin,T_end,alpha,L,Distance_matrix,Demand_matrix)#运行SA()，输出当次运行的最优长度和最优路线
    ...
    work_book.save('SA KILLS TSP ('+str(independent_runs)+' times).xls')        #保存工作表于项目目录下，命名为XXX
    plt.clf()                                                                   #清空画板
    plt.plot(Several_independent_runs_distance_list)                            #绘制若干次独立运行的最优路线长度折线图
    plt.savefig('SA KILLS TSP ('+str(independent_runs)+' times)')                                          #保存图片于项目目录下，命名为XXX
