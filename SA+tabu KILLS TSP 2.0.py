...
def SA(T_begin,T_end,alpha,L,Distance_matrix,Demand_matrix):                    #T_begin初始温度;T_end终止温度;alpha降温系数;L迭代次数
    ...
    plt.savefig("SA+tabu KILLS TSP A single run fitness.png")                   #保存到项目目录下命名为XXX
...
def main():
    ...
    work_book.save('SA + Tabu KILLS TSP ('+str(independent_runs)+' times).xls')                                   #保存工作表于项目目录下，命名为XXX
    plt.clf()                                                                   #清空画板
    plt.plot(Several_independent_runs_distance_list)                            #绘制若干次独立运行的最优路线长度折线图
    plt.savefig('SA + Tabu KILLS TSP ('+str(independent_runs)+' times')                                          #保存图片于项目目录下，命名为XXX
